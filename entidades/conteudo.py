class BaseConteudo:
    ...
    def calcular_total_interacoes_engajamento(self):
        ...
        return sum(1 for inter in self._interacoes if getattr(inter, 'tipo_interacao', None) in tipos_engajamento)
    ...
    def calcular_tempo_total_consumo(self):
        return sum(getattr(inter, 'watch_duration_seconds', 0) or 0 for inter in self._interacoes)

    # Calcula a média do tempo de consumo maior que zero
    def calcular_media_tempo_consumo(self):
        tempos = [getattr(inter, 'watch_duration_seconds', 0) or 0 for inter in self._interacoes if getattr(inter, 'watch_duration_seconds', 0)]
        if tempos:
    ...
    def listar_comentarios(self):
        return [getattr(inter, 'comment_text', '') for inter in self._interacoes if getattr(inter, 'tipo_interacao', None) == 'comment']
    ...
    def __repr__(self):
        return f"BaseConteudo(id={self._id_conteudo}, nome='{self._nome_conteudo}')"   